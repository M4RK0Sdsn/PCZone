...
class ContactosModel {
    ...
    public function buscarDatos($tabla, $query) {
        // Establecer la conexión
        $conn = Conex1::con1();
        ...
        switch ($tabla) {
            case 'empleados':
                $sql = "SELECT idEmpleado AS Id, nombre AS Nombre, apellidos AS Apellidos, salario AS Salario, puesto AS Puesto, departamento AS Departamento, email AS Email, telefono AS Telefono, fechaContratacion AS 'Fecha de Contratación', horasSemana AS 'Horas por Semana', nombreUsuario AS Usuario 
                        FROM empleados 
                        WHERE nombre LIKE ? OR apellidos LIKE ? OR puesto LIKE ? OR departamento LIKE ?";
                break;
            case 'clientes':
                $sql = "SELECT idCliente AS Id, nombre AS Nombre, apellidos AS Apellidos, correoElectronico AS Email, telefono AS Teléfono, direccion AS Dirección, anhoNacimiento AS 'Año de Nacimiento' 
                        FROM clientes 
                        WHERE nombre LIKE ? OR apellidos LIKE ? OR correoElectronico LIKE ? OR telefono LIKE ?";
                break;
            case 'proveedores':
                $sql = "SELECT idProveedor AS Id, nombre AS Nombre, direccion AS Dirección, correoElectronico AS Email, telefono AS Teléfono 
                        FROM proveedores 
                        WHERE nombre LIKE ? OR direccion LIKE ? OR correoElectronico LIKE ? OR telefono LIKE ?";
                break;
            default:
                return ['status' => 'error', 'message' => 'Tabla no válida']; // Retornar error si la tabla no es válida
        }
    ...
    public function actualizarEmpleado($eidEmpleado, $enombreEmpleado, $eapellidosEmpleado, $esalarioEmpleado, $epuestoEmpleado, $edepartamentoEmpleado, $eemailEmpleado, $etelefonoEmpleado, $efechaContratacion, $ehorasSemana, $enombreUsuario, $econtrasenaUsuario) {
        ...
        // Preparar la consulta SQL para actualizar los datos en la tabla empleados
